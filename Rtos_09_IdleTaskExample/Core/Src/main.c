...
const TickType_t _50ms = pdMS_TO_TICKS(50);  //funzione per convertire in Ticks un determinato numero di ms

const TickType_t _250ms = pdMS_TO_TICKS(250);

TaskProfiler BlueVar, GreenVar, RedVar , IdleTask;
...
//per attivare l'idle task bisogna attivarlo da freeRTOS config
...
void vBlueTask(void *pvParameters) {
	...
	while (1) {
		BlueVar++;
		vTaskDelay(_250ms);
		//per eseguire un task periodicamente
		//vTaskDelayUntil(&xLastWakeTime,_500ms);

		// per creare un pausa attraverso un block del task
		//vTaskDelay(_50ms);
	}
}
...
//funzione config Idle Task eseguita per prevenire consumo CPU
